...
/**
 * Generate "Better Together" insights based on partnership profile.
 * Identifies where SalesAI would complement the partner's offering.
 * Uses SALESAI_PROFILE for accurate positioning.
 */
function generateBetterTogetherInsights(profile, companyData) {
  const insights = {
    overlappingRoles: [],       // Roles both companies target
    complementaryUseCases: [],  // Use cases where SalesAI adds value
    industryOpportunities: [],  // Industries for joint GTM
    partnershipType: null,      // "referral", "reseller", "technology", "co-sell"
    gtmFit: null,               // How well GTM motions align
    customerJourney: [],        // How SalesAI fits in their customer journey
    salesAiUseCases: [],        // Relevant SalesAi use cases for this partner
    summary: ''
  };

  // Use the actual SalesAi profile
  const salesAiTargetRoles = SALESAI_PROFILE.targetCustomers.teams;
  const salesAiIndustries = SALESAI_PROFILE.targetCustomers.segments;

  // Find overlapping roles
  profile.targetRoles.forEach(role => {
    if (salesAiTargetRoles.some(r => role.toLowerCase().includes(r.toLowerCase().split(' ')[0]))) {
      insights.overlappingRoles.push(role);
    }
  });

  // Match partner to SalesAi use cases
  SALESAI_PROFILE.useCases.forEach(useCase => {
    // Check if partner's use cases relate to this SalesAi use case
    const relevantKeywords = {
      'speed-to-lead': ['pipeline', 'lead', 'response', 'inbound'],
      'inbound-response': ['inbound', 'call', 'support', 'phone'],
      'tier1-support': ['support', 'service', 'success', 'help'],
      'pipeline-revival': ['pipeline', 'nurture', 'engagement', 'crm'],
      'outbound-calling': ['outbound', 'prospecting', 'sdr', 'cold']
    };

    const keywords = relevantKeywords[useCase.id] || [];
    const isRelevant = profile.useCases.some(uc =>
      keywords.some(kw => uc.toLowerCase().includes(kw))
    ) || profile.targetRoles.some(role =>
      keywords.some(kw => role.toLowerCase().includes(kw))
    );

    if (isRelevant) {
      insights.salesAiUseCases.push(useCase);
    }
  });

  // If no specific matches, add the most common use cases
  if (insights.salesAiUseCases.length === 0) {
    insights.salesAiUseCases = SALESAI_PROFILE.useCases.filter(uc =>
      ['speed-to-lead', 'inbound-response'].includes(uc.id)
    );
  }

  // Find complementary use cases
  insights.salesAiUseCases.forEach(useCase => {
    insights.complementaryUseCases.push({
  ...
  profile.targetIndustries.forEach(industry => {
    const isHighPriority = salesAiIndustries.some(ind =>
      industry.toLowerCase().includes(ind.toLowerCase())
    );
    insights.industryOpportunities.push({
      industry,
      priority: isHighPriority ? 'high' : 'medium',
      reason: isHighPriority ? 'Strong overlap in target market' : 'Expansion opportunity'
    });
  });

  // Determine partnership type and get customer journey
  const partnerCategory = detectPartnerCategory(profile, companyData);
  const journeyMap = SALESAI_PROFILE.partnerComplementMap[partnerCategory];

  if (journeyMap) {
    insights.customerJourney = journeyMap.customerJourney;
    insights.betterTogetherStatement = journeyMap.betterTogether;
    insights.partnerValue = journeyMap.theirValue;
    insights.salesAiValue = journeyMap.ourValue;
  }

  // Determine partnership type
  if (profile.targetRoles.some(r => r.includes('Agencies') || r.includes('Consultants'))) {
    insights.partnershipType = 'reseller';
  } else if (partnerCategory) {
    insights.partnershipType = 'technology';
  } else if (insights.overlappingRoles.length >= 2) {
    insights.partnershipType = 'co-sell';
  } else {
    insights.partnershipType = 'referral';
  }

  // GTM fit assessment
  if (profile.gtmMotion === 'sales-led') {
    insights.gtmFit = 'strong';
  } else if (profile.gtmMotion === 'hybrid') {
    insights.gtmFit = 'good';
  } else {
    insights.gtmFit = 'moderate';
  }

  // Generate the unified story IN THE PARTNER'S VOICE
  const companyName = companyData?.name || 'Your company';
  const partnerVoiceStory = generateUnifiedStoryInPartnerVoice(profile, companyName, insights);

  insights.partnerVoiceStory = partnerVoiceStory;
  insights.summary = partnerVoiceStory.elevator || `${companyName} can refer customers who need AI-powered voice agents for sales and support.`;

  state.betterTogetherInsights = insights;

  return insights;
}

/**
 * Generates the SalesAi story IN THE PARTNER'S VOICE.
 *
 * This is the critical function that transforms generic SalesAi messaging
 * into language that resonates with this specific partner by:
 * 1. Mirroring their tagline structure
 * 2. Using their action verbs
 * 3. Framing pain points in their language
 * 4. Describing outcomes the way they describe outcomes
 *
 * The goal: When a partner reads this, it feels like THEIR story, not ours.
 */
function generateUnifiedStoryInPartnerVoice(profile, companyName, insights) {
  const language = profile.language || {};
  const story = {
    elevator: '',           // One-liner that sounds like them
    painStatement: '',      // Problem framed in their language
    outcomeStatement: '',   // Benefit in their outcome style
    valueProps: [],         // Key points using their verbs
    callToAction: '',       // Action in their voice
    customerStory: ''       // Joint value narrative
  };

  // Get their key verbs (or fall back to common ones)
  const verbs = language.actionVerbs?.length > 0
    ? language.actionVerbs
    : ['accelerate', 'automate', 'improve', 'boost'];

  // Get a relevant SalesAi use case for this partner
  const primaryUseCase = insights.salesAiUseCases?.[0] || SALESAI_PROFILE.useCases[0];

  // 1. ELEVATOR PITCH - Mirror their tagline structure
  if (language.tagline) {
    // Analyze their tagline structure and create a parallel one
    const tagline = language.tagline;

    // Common patterns to mirror:
    // "[Verb] your [noun]" -> "[Verb] your [response time]"
    // "[Achieve] [outcome]" -> "[Achieve] [instant engagement]"
    // "The [adjective] way to [verb]" -> "The [adjective] way to [respond]"

    if (/^(the|a)\s+\w+\s+way\s+to/i.test(tagline)) {
      // Pattern: "The [adj] way to [verb]"
      const adj = tagline.match(/^(?:the|a)\s+(\w+)\s+way/i)?.[1] || 'fastest';
      story.elevator = `${companyName} + SalesAi: The ${adj} way to capture and convert every lead`;
    } else if (/^\w+\s+your\s+\w+/i.test(tagline)) {
      // Pattern: "[Verb] your [noun]"
      const verb = language.actionVerbs?.[0] || 'Accelerate';
      story.elevator = `${companyName} + SalesAi: ${capitalize(verb)} your pipeline with instant voice engagement`;
    } else if (language.heroStatements?.length > 0) {
      // Use structure from hero statement
      const verb = language.actionVerbs?.[0] || 'Power';
      story.elevator = `${companyName} + SalesAi: ${capitalize(verb)} every conversation from first contact to close`;
    } else {
      // Default but personalized
      story.elevator = `${companyName} + SalesAi: Never miss a moment to engage your best prospects`;
    }
  } else {
    // No tagline extracted - use their verbs at minimum
    const verb = verbs[0] || 'Accelerate';
    story.elevator = `${companyName} + SalesAi: ${capitalize(verb)} revenue with AI-powered instant response`;
  }

  // 2. PAIN STATEMENT - Use their pain language patterns
  if (language.painPoints?.length > 0) {
    // Find patterns in their pain language
    const painSample = language.painPoints[0].toLowerCase();

    if (painSample.includes('stop') || painSample.includes('never')) {
      story.painStatement = `Never miss another hot lead. While prospects wait, competitors win. SalesAi answers in seconds, 24/7.`;
    } else if (painSample.includes('tired') || painSample.includes('frustrated')) {
      story.painStatement = `Tired of leads going cold before your team can respond? SalesAi engages instantly, every time.`;
    } else if (painSample.includes('losing') || painSample.includes('missing')) {
      story.painStatement = `Stop losing revenue to slow response times. SalesAi ensures every lead gets immediate attention.`;
    } else {
      story.painStatement = `Your best leads deserve instant attention. SalesAi makes it happen, 24/7.`;
    }
  } else {
    story.painStatement = `Speed wins deals. SalesAi responds to leads in seconds, not hours.`;
  }

  // 3. OUTCOME STATEMENT - Mirror their outcome language
  if (language.outcomes?.length > 0) {
    // Look for percentage/metric patterns
    const hasMetrics = language.outcomes.some(o => /\d+%|\d+x/.test(o));

    if (hasMetrics) {
      // They speak in metrics - we should too
      story.outcomeStatement = `Customers see 3x more conversations and 40% faster speed-to-lead when adding SalesAi to their ${companyName} stack.`;
    } else {
      // They speak in qualitative outcomes
      const verb = verbs[0] || 'improve';
      story.outcomeStatement = `${capitalize(verb)} every stage of your pipeline with AI that responds, qualifies, and books meetings automatically.`;
    }
  } else {
    story.outcomeStatement = `More conversations. Faster response. Better conversion. That's the SalesAi advantage.`;
  }

  // 4. VALUE PROPS - Using their action verbs
  story.valueProps = [
    `${capitalize(verbs[0] || 'Accelerate')} lead response from hours to seconds`,
    `${capitalize(verbs[1] || 'Automate')} qualification and meeting booking 24/7`,
    `${capitalize(verbs[2] || 'Integrate')} seamlessly with ${companyName} workflows`,
    `${capitalize(verbs[3] || 'Scale')} your outreach without adding headcount`
  ];

  // 5. CALL TO ACTION - Match their voice
  const ctaVerb = verbs.find(v => ['discover', 'see', 'start', 'get', 'try', 'experience'].includes(v.toLowerCase())) || 'discover';
  story.callToAction = `${capitalize(ctaVerb)} how ${companyName} + SalesAi can transform your customers' results.`;

  // 6. CUSTOMER STORY - Joint narrative
  if (insights.customerJourney?.length > 0) {
    const journeySteps = insights.customerJourney;
    story.customerStory = `Here's how it works: Your customer uses ${companyName} to ${journeySteps[0]?.step?.toLowerCase() || 'manage their pipeline'}. ` +
      `SalesAi handles the instant engagement - answering calls, qualifying leads, and booking meetings before competitors can respond. ` +
      `Together, you deliver a complete revenue acceleration solution.`;
  } else {
    story.customerStory = `${companyName} + SalesAi creates a complete pipeline acceleration solution. ` +
      `You provide the intelligence and strategy. We ensure every lead gets instant, intelligent engagement.`;
  }

  console.log('Generated partner-voice story:', {
    elevator: story.elevator,
    verbsUsed: verbs.slice(0, 4),
    hasCustomPain: !!language.painPoints?.length,
    hasCustomOutcomes: !!language.outcomes?.length
  });
...
/**
 * Detect what category of partner this is based on their profile
 */
function detectPartnerCategory(profile, companyData) {
  const useCasesLower = (profile.useCases || []).map(uc => uc.toLowerCase()).join(' ');
  const companyName = (companyData?.name || '').toLowerCase();

  // Check each category
  if (useCasesLower.includes('conversation') || useCasesLower.includes('call recording') ||
      useCasesLower.includes('coaching') || companyName.includes('gong') || companyName.includes('chorus')) {
    return 'Conversation Intelligence';
  }

  if (useCasesLower.includes('engagement') || useCasesLower.includes('sequence') ||
      useCasesLower.includes('cadence') || companyName.includes('outreach') || companyName.includes('salesloft')) {
    return 'Sales Engagement';
  }

  if (useCasesLower.includes('crm') || companyName.includes('hubspot') ||
      companyName.includes('salesforce') || companyName.includes('pipedrive')) {
    return 'CRM Platform';
  }

  if (useCasesLower.includes('chat') || useCasesLower.includes('visitor') ||
      companyName.includes('drift') || companyName.includes('qualified') || companyName.includes('intercom')) {
    return 'Conversational Sales';
  }
...
function displayPartnershipProfileCard(profile, insights) {
  ...
  // Build role tags
  const roleTags = profile.targetRoles.map(role => `
    <span class="profile-tag role-tag">${role}</span>
  `).join('');

  // Build industry tags
  const industryTags = profile.targetIndustries.map(ind => `
    <span class="profile-tag industry-tag">${ind}</span>
  `).join('');

  // Build use case tags
  const useCaseTags = profile.useCases.map(uc => `
    <span class="profile-tag usecase-tag">${uc}</span>
  `).join('');

  // GTM motion display
  const gtmDisplay = {
    'plg': { label: 'Product-Led', icon: '🚀', color: '#8b5cf6' },
    'sales-led': { label: 'Sales-Led', icon: '🤝', color: '#3b82f6' },
    'hybrid': { label: 'Hybrid', icon: '⚡', color: '#10b981' }
  };
  const gtm = gtmDisplay[profile.gtmMotion] || { label: 'Unknown', icon: '❓', color: '#6b7280' };

  // Company size display
  const sizeDisplay = {
    'smb': { label: 'SMB Focus', icon: '🏢' },
    'mid-market': { label: 'Mid-Market', icon: '🏛️' },
    'enterprise': { label: 'Enterprise', icon: '🏰' }
  };
  const size = sizeDisplay[profile.companySize] || null;

  // Partnership type styling
  const partnershipStyles = {
    'referral': { label: 'Referral Partner', color: '#f59e0b', desc: 'Introduce opportunities for commission' },
    'reseller': { label: 'Reseller Partner', color: '#8b5cf6', desc: 'White-label or bundle SalesAI' },
    'technology': { label: 'Technology Partner', color: '#3b82f6', desc: 'Integrate products for joint value' },
    'co-sell': { label: 'Co-Sell Partner', color: '#10b981', desc: 'Joint sales motion to shared ICP' }
  };
  const partnershipType = partnershipStyles[insights.partnershipType] || partnershipStyles['referral'];

  // Build complementary use cases section
  const complementarySection = insights.complementaryUseCases.length > 0 ? `
    <div class="profile-section">
      <div class="section-title">🤝 Complementary Capabilities</div>
      <div class="complementary-list">
        ${insights.complementaryUseCases.slice(0, 3).map(uc => `
          <div class="complementary-item ${uc.type}">
            <span class="comp-icon">${uc.type === 'overlap' ? '🔗' : '➕'}</span>
            <span class="comp-usecase">${uc.useCase}</span>
            <span class="comp-insight">${uc.insight}</span>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  messageDiv.innerHTML = `
    <div class="message-content">
      <div class="message-text">🎯 I found their go-to-market positioning — here's our "Better Together" story:</div>
      <div class="partnership-profile-card">
        <div class="profile-header">
          <div class="profile-title">
            <span class="profile-icon">🤝</span>
            <span>Partnership Profile</span>
          </div>
          <div class="partnership-type-badge" style="--type-color: ${partnershipType.color}">
        ...
        ${profile.targetRoles.length > 0 ? `
          <div class="profile-section">
            <div class="section-title">👥 Who They Sell To</div>
        ...
        ${profile.targetIndustries.length > 0 ? `
          <div class="profile-section">
            <div class="section-title">🏢 Industries They Serve</div>
            <div class="profile-tags">
              ${industryTags}
            </div>
          </div>
        ...
        ${profile.useCases.length > 0 ? `
          <div class="profile-section">
            <div class="section-title">🎯 Problems They Solve</div>
            <div class="profile-tags">
              ${useCaseTags}
            </div>
          </div>
        ...
        <div class="profile-metadata">
          ${profile.gtmMotion ? `
            <div class="metadata-item" style="--meta-color: ${gtm.color}">
              <span class="meta-icon">${gtm.icon}</span>
              <span class="meta-label">${gtm.label} GTM</span>
            </div>
          ` : ''}
          ${size ? `
            <div class="metadata-item">
              <span class="meta-icon">${size.icon}</span>
              <span class="meta-label">${size.label}</span>
            </div>
          ` : ''}
          ${insights.gtmFit ? `
            <div class="metadata-item gtm-fit-${insights.gtmFit}">
              <span class="meta-icon">${insights.gtmFit === 'strong' ? '✅' : insights.gtmFit === 'good' ? '👍' : '📊'}</span>
              <span class="meta-label">${insights.gtmFit.charAt(0).toUpperCase() + insights.gtmFit.slice(1)} GTM Fit</span>
            </div>
          ` : ''}
        </div>
        ...
        ${insights.summary ? `
          <div class="profile-summary">
            <div class="summary-title">💡 Partnership Opportunity</div>
            <p class="summary-text">${insights.summary}</p>
            <p class="partnership-desc">${partnershipType.desc}</p>
          </div>
        ` : ''}

        ${profile.pagesScraped.length > 0 ? `
          <div class="profile-footer">
            <span class="pages-scraped">Analyzed ${profile.pagesScraped.length} pages from their website</span>
          </div>
        ` : ''}
      </div>
    </div>
